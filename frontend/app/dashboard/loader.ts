 ...
 * * This module handles server-side data aggregation for the Matchmaking Dashboard.
 * Satisfaction numbers are truncated to 1 decimal place for UI clarity.
 */
...
async function dbQuery<T>(sql: string): Promise<T[]> {
  ...
  if (lastResponse?.status === "ERR") throw new Error(`SurrealDB Query Error: ${lastResponse.result}`);
...
export async function getDashboardData(): Promise<DashboardData> {
  try {
    const [appleCount, orangeCount, matchCount, matchData, globalStats] = await Promise.all([
      ...
      dbQuery<{ avg_apple: number; avg_orange: number; count: number }>(
        "SELECT math::mean(apple_score) AS avg_apple, math::mean(orange_score) AS avg_orange, count() AS count FROM matched_to GROUP ALL"
      ),
    ]);
    ...
    // Truncating satisfaction rates to 1 decimal place for cleaner UI
    const attributeStats: AttributeStat[] = [
      {
        attribute: "Apple Satisfaction",
        satisfactionRate: Math.round((stats?.avg_apple ?? 0) * 1000) / 10,
        sampleSize: stats?.count ?? 0
      },
      {
        attribute: "Orange Satisfaction",
        satisfactionRate: Math.round((stats?.avg_orange ?? 0) * 1000) / 10,
        sampleSize: stats?.count ?? 0
      }
    ];
    ...
    return {
      metrics: { 
        ...
        totalOranges, 
        totalMatches, 
        avgMutualScore: matchData.length > 0 ? Math.round((matchData.reduce((s, m) => s + (m.score ?? 0), 0) / matchData.length) * 1000) / 1000 : 0,
        // Using toFixed(3) equivalent math for the card metrics
        avgAppleScore: Math.round((stats?.avg_apple ?? 0) * 1000) / 1000,
        avgOrangeScore: Math.round((stats?.avg_orange ?? 0) * 1000) / 1000
      },
      recentMatches: matchData.slice(0, 10).map(m => ({
        id: String(m.id), 
        appleId: String(m.in), 
        orangeId: String(m.out),
        score: m.score, 
        appleScore: m.apple_score, 
        orangeScore: m.orange_score,
    ...
    console.error("[Loader] Failed to fetch dashboard data:", err);
    return {
